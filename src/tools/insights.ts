...
Returns raw data for you to interpret — present findings honestly and avoid overclaiming causation.`,
  ...
  execute: async (
    _id: string,
    params: { analysis_type: string; days_back?: number }
  ) => {
    const db = getDb();
    ...
    switch (params.analysis_type) {
      case "stress_vs_sleep": {
        ...
        result = {
          analysis: "stress_vs_sleep", days, detail_data: data,
          after_stress: avgWithStress, no_stress: avgWithoutStress,
      ...
      case "alcohol_vs_hrv": {
        ...
        result = {
          analysis: "alcohol_vs_hrv", days, detail_data: data,
          after_alcohol: avgWithAlcohol, no_alcohol: avgWithoutAlcohol,
          sample_size: data.length,
        };
        break;
      }
      ...
      case "exercise_vs_readiness": {
        ...
        result = { analysis: "exercise_vs_readiness", days, data, sample_size: data.length };
        break;
      }
      ...
      case "sleep_levers": {
        ...
        const median = sleepData.length > 0
        ...
        result = {
          analysis: "sleep_levers", days,
          sleep_overview: { total_nights: sleepData.length, threshold_used: threshold },
          before_good_sleep: beforeGoodSleep, before_bad_sleep: beforeBadSleep,
      ...
      case "weekly_trends": {
        const weeks = db
          .prepare(
            `SELECT strftime('%Y-W%W', logged_at) as week, category,
                    COUNT(*) as entries, AVG(value) as avg_value
             FROM log_entries WHERE logged_at >= datetime('now', ?)
             GROUP BY week, category ORDER BY week, category`
          )
          .all(since);
        ...
        const ouraWeeks = db
          .prepare(
            `SELECT strftime('%Y-W%W', date) as week,
                    AVG(sleep_score) as avg_sleep, AVG(readiness_score) as avg_readiness,
                    AVG(hrv_average) as avg_hrv, AVG(steps) as avg_steps
             FROM oura_daily WHERE date >= date('now', ?)
             GROUP BY week ORDER BY week`
          )
          .all(since);

        result = { analysis: "weekly_trends", days, log_trends: weeks, oura_trends: ouraWeeks };
        break;
      }
      ...
      case "streak_report": {
        const categories = db
          .prepare(
            `SELECT DISTINCT category FROM log_entries WHERE logged_at >= datetime('now', '-90 days')`
          )
        ...
        for (const { category } of categories) {
          const dates = db
            .prepare(
              `SELECT DISTINCT date(logged_at) as d FROM log_entries
               WHERE category = ? ORDER BY d DESC`
            )
          ...
          streaks[category] = { current_streak_days: currentStreak, total_entries_90d: dates.length };
        }

        result = { analysis: "streak_report", streaks };
        break;
      }
